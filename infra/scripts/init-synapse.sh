...
: "${TURN_SHARED_SECRET:=}"
# Host TURN alcanzable por los clientes (por defecto, el server_name).
: "${TURN_HOST:=$MATRIX_DEFAULT_SERVER_NAME}"
: "${INTERNAL_API_SECRET:=}"
: "${ISOLATION_EXEMPT_USERS:=}"
export MATRIX_DEFAULT_SERVER_NAME APP_PUBLIC_URL MATRIX_REGISTRATION_SHARED_SECRET
export RECAPTCHA_PUBLIC_KEY RECAPTCHA_PRIVATE_KEY TURN_SHARED_SECRET TURN_HOST
...
envsubst '${MATRIX_DEFAULT_SERVER_NAME} ${APP_PUBLIC_URL} ${MATRIX_REGISTRATION_SHARED_SECRET} ${RECAPTCHA_PUBLIC_KEY} ${RECAPTCHA_PRIVATE_KEY} ${TURN_SHARED_SECRET} ${TURN_HOST} ${INTERNAL_API_SECRET} ${ISOLATION_EXEMPT_USERS}' \
